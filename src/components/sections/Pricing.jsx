      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="text-center mb-16">
          ...
          <div className="flex items-center justify-center gap-4">
            <span className={`text-sm ${!isAnnual ? 'text-gray-900 font-semibold' : 'text-gray-400'}`}>Bill Monthly</span>
            <button 
              onClick={() => setIsAnnual(!isAnnual)}
              className="w-12 h-6 bg-[#55D3A7] rounded-full relative transition-all"
            >
              <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${isAnnual ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
            <span className={`text-sm ${isAnnual ? 'text-gray-900 font-semibold' : 'text-gray-400'}`}>Bill Annually</span>
          </div>
        </div>
